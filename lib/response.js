...
var Response = {
	...
	error: function(err, status, calvin){

		console.log('Error status: ' + status);

		status = ['403','404','500','502','503'].contains(status.toString()) ? status : '500';
		...
		if (calvin.environment === 'production') {
			calvin.router.routes['get']['/' + status].fn(this.request, this);
		} else {
			var view = {
				error: err
			};
