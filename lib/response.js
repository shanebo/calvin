    ...
    serve: function(asset, cache){
        ...
        if (asset.headers['Content-Type'] == 'text/html') {
            asset.headers['Content-Length'] = Buffer.byteLength(asset.body, 'utf8');
                ...
                if (request.method == 'GET' && !this.app.cache[request.url] && this.memcache.regex && (cache === true || this.memcache.regex.test(request.url))) {
                    this.memcache.add(request.url);
                    this.app.cache[request.url] = asset;
                }
        }
