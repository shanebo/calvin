...
exports.extend = function(request, response){
    ...
    request.query = {};
    request.params = {};
    request.xhr = (request.headers['x-requested-with'] || '').toLowerCase() === 'xmlhttprequest';
    request.ip = (request.headers['x-forwarded-for'] || '').split(',')[0] || request.connection.remoteAddress;
    request.ips = (request.headers['x-forwarded-for'] || '').split(',');
};
