

var fs = require('fs');
var mime = require('mime');
var uglifyjs = require('uglify-js');
var sqwish = require('sqwish');
var zlib = require('zlib');


var Assets = function(options){
	console.log('--> Initializing Cache middleware...');
	this.options = options;

	this.options.cache.forEach(function(dir){
		this.cacheDirectory(dir);
	}.bind(this));

	this.combineAssets();
	this.storeFile(__dirname + '/../error.html', '/views/error.html');
};


Assets.prototype = {

	cacheDirectory: function(dir){
		var directory = this.options.directory + dir;

		fs.readdirSync(directory).forEach(function(item){
			if (item.charAt(0) === '.') return;

			var path = directory + item;
			var key = dir + item;

			if (fs.statSync(path).isDirectory()) this.cacheDirectory(key + '/')
			else this.storeFile(path, key);
		}.bind(this));
	},

	minFile: function(path, type){
		var data = fs.readFileSync(path, 'UTF-8');

		switch(type) {
			case 'text/css': 				return new Buffer(sqwish.minify(data));
			case 'application/javascript': 	return new Buffer(uglifyjs.minify(data, {fromString: true}).code);
			case 'text/html': 				return data;
			default: 						return fs.readFileSync(path);
		}
	},

	buildAsset: function(type, stats, body){
		return {
			headers: {
				'Content-Type': type,
//				'Content-Length': body.length,
//				'length': type == 'text/html' ? new Buffer(body).length : body.length,
				'ETag': '"' + stats.mtime.toUTCString() + '"',
				'Last-Modified': stats.mtime.toUTCString(),
				'Cache-Control': 'public'
//				,
//				'Cache-Control': 'public, max-age=' + this.options.max_age
			},
			body: body
		};
	},

	storeFile: function(path, key){
		// do a find and replace regex on ../../ so I can load up higher level universally used assets
		var type = mime.lookup(path);
		var body = this.minFile(path, type);
		var stats = fs.statSync(path);

		this.options.cache[key] = this.buildAsset(type, stats, body);

		// new gzip
		if (type != 'text/html') {
			var asst = this.options.cache[key];
			zlib.gzip(asst.body, function(err, buffer){
				if (err) throw err;
				asst.gzip = buffer;
			});
		}

		if (this.options.environment == 'development') {
			fs.unwatchFile(path);
	
			fs.watchFile(path, function(prev, curr){
				if (curr.mtime.getTime() - prev.mtime.getTime()) {
					console.log('Recaching new ' + path + ' file');
					this.storeFile(path, key);
					this.combineAssets(path.replace(this.options.directory + '/public',''));
				}
			}.bind(this));
		}
	},

	combineAssets: function(file){
		for (var route in this.options.combine) {
			var files = this.options.combine[route];
			if (file == null || (file && files.contains(file))) this.combine(route, files);
		}
	},

	combine: function(route, files){
		console.log('\n\nCombining ' + files + ' to ' + route + ' route');
		var results = '';

		files.forEach(function(path){
			results += this.options.cache['/public' + path].body.toString('UTF-8');
		}.bind(this));

		var buff = new Buffer(results);
		this.options.cache['/public' + route] = this.buildAsset(mime.lookup(route), {mtime: new Date()}, buff);

		// gzip the combined results
		// and set the gzip property of the cached asset
		var asst = this.options.cache['/public' + route];

		if (asst['Content-Type'] != 'text/html') {

			zlib.gzip(buff, function(err, buffer){
			    if (err) throw err;
				asst.gzip = buffer;
// ADD CONTENT LENGTH HERE
			});
		}
	},

/*
	serve: function(request, response, asset){
		if (request.headers['if-none-match'] && (request.headers['if-none-match'] == asset.headers['ETag'])) {
			response.writeHead(304, asset.headers);
			response.end();
			return;
		}


		if (asset.headers['Content-Type'] !== 'text/html') {
			asset.headers['Content-Encoding'] = 'gzip';
//			asset.headers['Transfer-Encoding'] = 'gzip; chunked';
		}
//		if (asset.headers['Content-Type'] !== 'text/html') asset.headers['Accept-Encoding'] = 'compress, gzip, deflate';
		asset.headers['Date'] = new Date().toUTCString();
		response.writeHead(200, asset.headers);

		if (asset.hasOwnProperty('gzip')) {
//		if (asset.headers['Content-Type'] !== 'text/html') {
//			console.log('!!!!!!! GZIPPED !!!!!!!!');
			response.end(asset.gzip);
		} else {
//			console.log('!!!!!!! BODY !!!!!!!!');
			response.end(asset.body);
		}
	},
*/

	handler: function(request, response, next){
		var url = request.url;
		var asset = this.options.cache['/public' + url];

//		console.log(url);
//		console.log('!!!!!!! IS IN ASSETS HANDLER !!!!!!!!');
//
//		console.log('\n');
//		console.log('this.options.publicDirs');
//		console.log(this.options.publicDirs);
//		console.log(url.split('/')[1].test(this.options.publicDirs));
//		console.log('\n');


		if (asset && !url.test(/views/)) {
			request.calvin.asset = 'memcache';
//			this.serve(request, response, file);
			response.serve(asset);

		} else if (url.split('/')[1].test(this.options.publicDirs)) {

//			console.log(url);
//			console.log('!!!!!!! IS AN UNCACHED STATIC FILE !!!!!!!!');
		
			var path = this.options.directory + '/public' + unescape(url);

			fs.readFile(path, function(err, contents){
//				console.log('!!!!!!! READING UNCACHED STATIC FILE !!!!!!!!');

				if (err) {
					response.writeHead(404);
					response.end();
				} else {
//					console.log('!!!!!!! GOT INSIDE SUCCESS !!!!!!!!');
					request.calvin.asset = 'uncached';

//					fs.stat(path, function(err, stats){
//						var asset = this.buildAsset(mime.lookup(path), stats, contents);
//						this.serve(request, response, asset);
						response.end(contents);
//					}.bind(this));
				}
			}.bind(this));

		} else if (next) {
			console.log('!!!!!!! ASSET DIDNT MATCH MOVING ON !!!!!!!!');
			next();
		}
	}

};


module.exports = Assets;