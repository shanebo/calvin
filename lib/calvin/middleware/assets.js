...
var Assets = function(options){
	...
	this.combineAssets();
	this.storeFile(__dirname + '/../public/error.html', '/views/error.html');
	this.storeFile(__dirname + '/../public/403.html', '/views/403.html');
	this.storeFile(__dirname + '/../public/404.html', '/views/404.html');
};
...
//				'Cache-Control': 'public, max-age=' + this.options.max_age
	...
	storeFile: function(path, key){
		...
		// new gzip
		if (type != 'text/html') {
			var asst = this.options.cache[key];
			zlib.gzip(asst.body, function(err, buffer){
				if (err) throw err;
				asst.gzip = buffer;
			});
		}
	...
	combine: function(route, files){
		...
		var asst = this.options.cache['/public' + route];

		if (asst['Content-Type'] != 'text/html') {
...
// ADD CONTENT LENGTH HERE
	...
	handler: function(request, response, next){
		...
		var asset = this.options.cache['/public' + url];

		// if a cached asset --> response.serveCached
		// else if url is equal to a public directory --> response.renderStatus 403
		// else if its a public route --> response.serveUncached
		// else if next, fire next
...
//		} else if (url.split('/')[1].test(this.options.publicDirs)) {
		} else if (url.test(this.options.publicDirs)) {

			if (url.replace(/\/$/, '').split('/').length == 2) {
				// trying to access a public folder so 403
				response.renderStatus(403);
				return;
			}
		
			var path = this.options.directory + '/public' + url;
