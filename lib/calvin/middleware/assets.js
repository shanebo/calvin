...
//			case 'text/html': 				return data.clean();
	...
	buildAsset: function(type, stats, body){
		// make this headers/body to the server method is much cleaner
		// { headers: {}, body: buffer }
		return {
			'Content-Type': type,
...
//					this.combineAssets(type === 'text/css' ? 'css' : 'js');
	...
	combineAssets: function(file){
		for (var route in this.options.combine) {
			...
			console.log('file && files.contains(file) ' + (file && files.contains(file)));
...
//		var content_type = (type == 'css') ? 'text/css' : 'application/javascript';
		var content_type = mime.lookup(route);
		console.log('content_type ' + content_type);
		this.options.cache['/public' + route] = this.buildAsset(content_type, {mtime: new Date()}, buff);
	},

	serve: function(response, asset){
