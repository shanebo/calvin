...
Assets.prototype = {
	...
	store: function(path, key){
		...
		this.app.cache[key] = this.buildAsset(type, stats, body);
	...
	press: function(file){
		for (var key in this.app.combine) {
			...
			if (file == null || (file && files.contains(file))) {
				...
				this.app.cache[key] = this.buildAsset(mime.lookup(key), { mtime: new Date() }, body);
			}
		}
	},
	...
	handler: function(request, response, next){
		...
		if (asset && request.method === 'GET' && !url.test(/^\/views/)) {
