...
var Assets = function(app){
	...
	this.app = app;
	this.app.publics = this.publicRegex('/public/');
	this.defaults();
	this.app.memcache.assets.push('/views/');
	this.app.memcache.assets.forEach(this.cache, this);
	this.press();
}
...
Assets.prototype = {
	...
	walkPublic: function(dir, dirs, files){
		fs.readdirSync(this.app.directory + dir).forEach(function(item){
			if (item.charAt(0) === '.') return;
			var path = this.app.directory + dir + item;
			var key = dir + item;
	...
	publicRegex: function(dir){
		var pub = this.app.directory + dir;
		...
		for (var route in this.app.combine) files.push(route);

		var folders = dirs.slice(0);
		folders.push(dir);

		folders.forEach(function(f){
			fs.unwatchFile(this.app.directory + f);
			fs.watchFile(this.app.directory + f, function(prev, curr){
				if (curr.mtime.getTime() - prev.mtime.getTime()) {
					this.app.publics = this.publicRegex(f);
					console.log('Publics updated to:');
					console.log(this.app.publics);
				}
			}.bind(this));
		}, this);
	...
	store: function(path, key){
		...
		this.app.cache[key] = this.buildAsset(type, stats, body, path.contains('.scss') ? body : null);
	...
	press: function(file){
		for (var key in this.app.combine) {
			var files = this.app.combine[key];
			if (file == null || (file && files.contains(file))) {
				...
				files.forEach(function(path, f){
					body += ' ' + this.app.cache[path].string;
				...
				this.app.cache[key] = this.buildAsset(mime.lookup(key), { mtime: new Date() }, body, scss);
			}
		}
	},
	...
	handler: function(request, response, next){
		...
		var asset = this.app.cache[url];

		if (asset && request.method === 'GET' && !url.test(/^\/views|^\/assets/)) {
			request.calvin.asset = 'memcached';
			response.serve(asset);

		} else if (this.app.publics.files.test(url)) {
			request.calvin.asset = 'uncached';
			var file = this.app.directory + '/public' + url;
			fs.createReadStream(file).pipe(response);

		} else if (this.app.publics.directories.test(url)) {
