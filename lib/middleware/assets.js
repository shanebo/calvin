...
const Assets = function(app){
    ...
    this.minify = app.settings.assets.minify;
    this.watch = app.settings.assets.watch;
    this.pressPaths = app.settings.assets.press;
    this.public = {
        files: {},
        directories: {}
    };

    // add press paths to public files
    for (var route in this.pressPaths) {
...
Assets.prototype = {
    ...
    store: function(path, key){
        ...
        this.cache[key] = this.buildAsset(type, stats, body, path.includes('.less') ? body : null);
    ...
    press: function(file){
        for (var key in this.pressPaths) {
            var files = this.pressPaths[key];
            if (file == null || (file && files.includes(file))) {
                ...
                this.cache[key] = this.buildAsset(mime.lookup(key), { mtime: new Date() }, body, lesscss);
            }
        }
    },
